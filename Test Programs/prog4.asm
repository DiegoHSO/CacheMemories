...
main:		nop
		...
		la		$t1, L1		# Carrega endereço da primeira linha em $t1 - PSEUDO-INSTRUÇÃO
		lw		$t2,0($t0)	# Lê valor da linha para $t0
		...
		addiu		$t4, $t4, 1 # Soma em 1 o contador da coluna
		j		linha
		
coluna: 	beq		$t4, $t3, fim # Caso tenha operado em todas as colunas, fim do programa
		addiu		$t1, $t1, 4  # Incrementa o ponteiro da matriz para a próxima coluna
		addu		$t0, $t1, $zero # Armazena o ponteiiro de $t1 em $t0
		lw		$t2, 0($t1) # Carrega o conteúdo da posição da matriz em $t2
		addiu		$t4, $t4, 1 # Soma em 1 o contador da coluna
		addu		$t5, $zero, $zero # Reinicializa o registrador $t5 em 0	
		
linha:		beq		$t5, $t3, coluna # Caso tenha terminado de somar os conteúdos da coluna, vai para a próxima
		...
		addiu		$t0, $t0, 40 # Incrementa o ponteiro da matriz para a próxima linha
		lw 		$t2,0($t0) # Carrega o conteúdo da posição da matriz em $t2
		j linha # Repete o laço
